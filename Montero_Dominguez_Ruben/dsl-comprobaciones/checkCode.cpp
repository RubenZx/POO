...
void checkCode::apply_clase(const MatchFinder::MatchResult &Result){
  if (const clang::CXXRecordDecl *Node = Result.Nodes.getNodeAs<clang::CXXRecordDecl>("classWithName")){
    map<string,bool>::iterator it = clases.find(Node->getNameAsString());
    if(it != clases.end())
      it->second = true;
  }
}
...
bool checkCode::findClass(vector<string> className) {
  ...
  if(ficheroValido==true){
    ...
    for(vector<string>::iterator it = className.begin() ; it != className.end(); ++it){
      ...
      clases.insert(std::pair<string,bool>(*it,false));
    }
    ClangTool Tool(OptionsParser.getCompilations(),p);
    Tool.run(newFrontendActionFactory(&FinderClase).get());
    for(std::pair<const string,bool>& c : clases){
      clase_encontrada = clase_encontrada && c.second;
    }
    if(this->foundClass()){
...
void checkCode::methodWithDynamicCast(string methodName, vector<string> parameters, string constant, string className, unsigned int numberdc, bool lenient, string message) {
  e.setDynamicCast(methodName,parameters,constant,className,numberdc,lenient,message);
