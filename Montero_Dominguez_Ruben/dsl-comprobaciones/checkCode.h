...
****************************************************************************/
/*
- Método: bool findClass(vector<string> className)
...
- Parámetros:
---vector<string> className (*):vector de cadena de caracteres con el nombre de las clases a buscar.
...
/*
- Método: void numberOfDynamicCast(string methodName, vector<string> parameters, string constant, string className, unsigned int numberdc, bool lenient, string message)

- Utilidad: Método para comprobar si un método usa dynamic cast y la cantidad

- Parámetros:
---string methodName (*): cadena de caracteres con el nombre del método que debe usar dynamic_casst
---vector<string> parameters: vector con los parámetros del método
---string constant: cadena que indica si el método es constante o no
---string className: cadena que indica la clase a la que pertenece el método
---unsigned int numberdc: entero sin signo que indica el número de llamadas a dynamic_cast que el método hace
---lenient: booleano que indica si se permite un número igual o mayor de constructores (true) o solo un número igual de constructores (false). Por defecto a "false".
...
- Ejemplo:
	...
	void A::metodo(Tipo2 *t2, Tipo3 *t3){
		Tipo1 *t1 = dynamic_cast<Tipo1>(t2);
		Tipo1 *t1 = dynamic_cast<Tipo1>(t3);
	}

	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",2,false);	No daría error
	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",2,true);		No daría error
	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",1,true);		No daría error
	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",1,false);	Daría error
	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",3,false);	Daría error
	methodWithDynamicCast("metodo",{"class Tipo2 *","class Tipo3 *"},"noconst","A",4,true);		Daría error

*/
	void methodWithDynamicCast(string methodName, vector<string> parameters, string constant, string className, unsigned int numberdc, bool lenient=false, string message = "INCORRECT NUMBER OF DYNAMIC_CAST");
...
/*
- Método: void allPrivateVariableMember(string className, string message)

- Utilidad: Método para buscar verificar que todas los miembros de una clase son privados.
...
Dado:
	...
	allPrivateVariableMember("A"); Daría error
	allPrivateVariableMember("B"); No daría error
*/
...
---vector<string> mainConstant (?): vector de cadena de caracteres para indicar si los métodos principales a buscar son constantes o no.
...
---vector<string> usedConstant (?): vector de cadena de caracteres para indicar si los métodos referenciados a buscar son constantes o no.
...
---string className (*): cadena de caracteres con el nombre de la clase donde debemos buscar los métodos deseados.
---vector<string> constant (?): vector de cadena de caracteres para indicar si los métodos a buscar son constantes o no.
---vector<unsigned int> numDefaultArgs: vector de enteros para indicar el número de argumentos por defecto que los métodos a buscar deben tener.
---vector<vector<string> > defaultArgs: vector de vector de string para indicar los valores por defecto.
...
- Ejemplo:
	...
	defaultArgumentsInMethod({"metodo1"}, {{"Tipo1"}}, "A", {"?"}, {1});		daría error
	defaultArgumentsInMethod({"metodo2"}, {{"Tipo1", "Tipo2"}}, "A", {"?"}, {2});	Daría error
	defaultArgumentsInMethod({"metodo2"}, {{"Tipo1", "Tipo2"}}, "A", {"?"}, {1}, {"0"}); No daría error
	defaultArgumentsInMethod({"metodo2"}, {{"Tipo1", "Tipo2"}}, "A", {"?"}, {1}, {"?"}); No daría error
...
/*
- Método:  void virtualMethod(vector<string> methodNames, vector<vector<string>> parameters, string className, vector<string> constant, string message);

- Utilidad: Método para comprobar que los métodos de una clase están marcado como virtuales
...
- Ejemplo:
	...
	virtualMethod({"A"}, {{"?"}}, "A", {"?"});			Daría error
	virtualMethod({"A"}, {{"const class A &"}}, "A", {"?"});	No daría error
...
/*
- Método: void methodWithReferencedFunction(vector<string> mainMethodNames, vector<vector<string> > mainMethodparameters, string mainClassName, vector<string> mainConstant, vector<string> usedFunctionNames, vector<vector<string> > usedFunctionParameters, string message)

- Utilidad: Método para validar si un determinado metodo está haciendo referencia a una función.
...
- Ejemplo:
	...
	methodWithReferencedFunction({"metodo1"}, {{"?"}}, "A", {"?"}, {"funcion1"}, {{"?"}}); No daría error
	methodWithReferencedFunction({"metodo1"}, {{"?"}}, "A", {"?"}, {"funcion3"}, {{"?"}}); Daría error
...
/*
- Método: void functionWithReferencedMethod(vector<string> mainFunctionNames, vector<vector<string> > mainFunctionParameters, vector<string> usedMethodNames, vector<vector<string> > usedMethodparameters, string usedClassName, vector<string> usedConstant,  string message)

- Utilidad: Método para validar si una determinada función está haciendo referencia a un método.
...
- Ejemplo:
	...
	functionWithReferencedMethod({"funcion1"}, {{"?"}}, {"metodo1"}, {{"?"}}, "A", {"?"}); No daría error
	functionWithReferencedMethod({"funcion1"}, {{"?"}}, {"metodo2"}, {{"?"}}, "A", {"?"}); Daría error
...
****************************************************************************/
	...
	bool clase_encontrada = true;
	std::map<std::string, bool> clases;
	string fileClass = "";
